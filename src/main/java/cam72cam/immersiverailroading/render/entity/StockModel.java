...
public class StockModel extends OBJRender {
	...
	public void draw(EntityRollingStock stock, float partialTicks) {
		...
		} else if (stock instanceof LocomotiveNuclear) {
			drawNuclearLocomotive((LocomotiveNuclear)stock);
	...
	private void drawNuclearLocomotive(LocomotiveNuclear stock) {
		if (stock.getTransmission() == LocomotiveNuclearDefinition.Transmissions.ELECTRICAL) {
			EntityRollingStockDefinition def = stock.getDefinition();
		...
		if (stock.getTransmission() == LocomotiveNuclearDefinition.Transmissions.MECHANICAL){
			LocomotiveNuclearDefinition def = stock.getDefinition();
	...
	private void drawDrivingWheels(LocomotiveNuclear stock, List<RenderComponent> wheels) {
		for (RenderComponent wheel : wheels) {
			double circumference = wheel.height() * (float) Math.PI;
			double relDist = distanceTraveled % circumference;
			double wheelAngle = 360 * relDist / circumference;
	...
	// NUCLEAR PISTON/MAIN/SIDE
		private void drawStephenson(LocomotiveNuclear stock, String side, int wheelAngleOffset, double diameter, Vec3d wheelCenter, Vec3d wheelPos) {
			LocomotiveNuclearDefinition def = stock.getDefinition();
		...
		private void drawT1(LocomotiveNuclear stock, String side, int wheelAngleOffset, double diameter, Vec3d wheelCenter, Vec3d wheelPos) {
			LocomotiveNuclearDefinition def = stock.getDefinition();
		...
		private RenderComponent requireComponent(LocomotiveNuclearDefinition def, RenderComponentType rct, String side, Gauge gauge) {
			RenderComponent comp = def.getComponent(rct, side, gauge);
			if (comp == null) {
				ImmersiveRailroading.error("Missing component for %s: %s %s", def.name(), rct, side);
			}
		...
		private void drawWalschaerts(LocomotiveNuclear stock, String side, int wheelAngleOffset, double diameter, Vec3d wheelCenter, Vec3d wheelPos) {
			LocomotiveNuclearDefinition def = stock.getDefinition();
